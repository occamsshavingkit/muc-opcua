...
- [ ] T005 [US2] Add `free()` to variant array encoding callers — found at `src/services/read.c` read_result path where allocated variant buffer must be freed after encoding (HP1, FR-004)
...
- [ ] T015 [P] [US2] Replace O(N^2) startup duplicate node check with sort-then-scan approach (qsort node IDs then linear scan) in `src/address_space/address_space.c` (HP11, FR-014)
- [ ] T016 [P] [US2] Parse MessageSize once and store in chunk context; remove second parse site in `src/core/message_chunk.c` (HP12, FR-015)
...
- [ ] T027 [P] [US3] Replace `calloc` with static/stack buffers in non-startup code paths; gate remaining `calloc` behind `MUC_OPCUA_ALLOW_HEAP` flag (UB7, FR-026)
...
- [ ] T036 [P] [US5] Fix circular verification in response encoding tests in `tests/unit/` — replace encode-then-decode-self patterns with comparison against independent binary fixture bytes (FR-032, TQ8)
- [ ] T037 [P] [US5] Audit each `*_encode` unit test file for mandatory field verification: check that each output field is asserted; add missing assertions in `tests/unit/` (FR-035)
...
T001 (baseline)
 ...
 ├─ US5 (T032-T040) ── independent, T037a audits then T040 fills gaps
 ├─ US6 (T041-T043) ── independent, all [P]
 └─ T044-T051 (polish) ── depends on all above
```
...
# US5: T032, T032a, T033, T034, T035, T036, T037, T037a, T038, T039, T040
#   (T037a must precede T040 for gap analysis)
# US6: T041, T042, T043
...
| 6: Tests/Interop | T032-T040 (incl. T032a, T037a) | US5 | P3 | 11 |
