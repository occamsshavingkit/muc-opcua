...
- **FR-005**: The crypto gate MUST default off for the nano profile and on for any profile/build that provides a crypto backend; the exact default policy MUST be driven by a Kconfig symbol (not solely by backend presence) so profile intent, not just toolchain availability, decides it. [NEEDS CLARIFICATION during planning: whether micro/embedded also default the gate off when built without a backend, or always on.]
