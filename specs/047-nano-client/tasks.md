...
- [X] T001 [P] Add `MUC_OPCUA_CLIENT_PROFILE` CMake option to `src/CMakeLists.txt`
  following the server's `MUC_OPCUA_PROFILE` pattern — nano profile enables only
  client connection + read + browse services.
- [X] T002 [P] Create `include/muc_opcua/client.h` with forward declarations,
...
- [X] T003 [P] Create `src/client/` directory structure with stub .c files for
  client.c, client_secure_channel.c, client_session.c, client_read.c,
  client_browse.c, client_state.c, client_errors.c
- [X] T004 [P] Define `mu_client_config_t`, `mu_client_t`, `mu_identity_token_t`,
  and `mu_client_transport_t` in `include/muc_opcua/client.h` per contracts/api.md
- [X] T004A [P] Reject illegal client/server profile combinations with `#error`
  in `include/muc_opcua/features.h`

## Phase 2: Foundational (Blocking Prerequisites)

**Purpose**: Core infrastructure before user story work

- [X] T005 [P] Implement client state machine in `src/client/client_state.c`
...
- [X] T006 [P] Implement `mu_client_init` in `src/client/client.c` — validate
  config, store transport pointer, initialize state to DISCONNECTED
- [X] T007 [P] Implement host TCP adapter (`mu_client_transport_t`) using POSIX
...
- [ ] T009 [P] [US1] Add connect/session tests in `tests/unit/test_nano_client.c`
  — connect to muc-opcua server with Basic256Sha256, get endpoints, open channel,
  create + activate session, close session, disconnect — verify all StatusCodes
  are GOOD (OPC-10000-4 §5.4.2, §5.5.2-3, §5.6.2-4; OPC-10000-6 §7.1.2.3/§7.1.2.4)
- [ ] T010 [P] [US1] Add SecurityPolicy None test in `tests/unit/test_nano_client.c`
  — connect with None and verify channel opens without crypto (LOW-005)
- [ ] T011 [P] [US1] Add CloseSecureChannel-only test in `tests/unit/test_nano_client.c`
  — open channel then close without creating session, verify clean disconnect
  (MEDIUM-003)
- [ ] T012 [P] [US1] Add connection loss test in `tests/unit/test_nano_client.c`
  — simulate TCP drop, verify client returns Bad_ConnectionClosed (FR-015)

### Implementation for User Story 1

- [ ] T013 [US1] Implement `mu_client_connect` in `src/client/client.c` — resolve
  ...
  chains T006+T007 + T015 into a single connect call (HIGH-003)
- [ ] T014 [US1] Implement `mu_client_get_endpoints` in `src/client/client.c`
  — construct GetEndpoints request, send via SecureChannel, parse response
  (OPC-10000-4 §5.4.2)
- [ ] T015 [US1] Implement OpenSecureChannel in `src/client/client_secure_channel.c`
...
- [ ] T016 [US1] Implement CloseSecureChannel in `src/client/client_secure_channel.c`
  — construct and send close message (OPC-10000-4 §5.5.3)
- [ ] T017 [US1] Implement `mu_client_create_session` in `src/client/client_session.c`
...
- [ ] T018 [US1] Implement `mu_client_activate_session` in `src/client/client_session.c`
  — construct ActivateSession request with `mu_identity_token_t` (anonymous),
  send, parse response (OPC-10000-4 §5.6.3)
- [ ] T019 [US1] Implement `mu_client_close_session` in `src/client/client.c`
  — send CloseSession request, delete subscription flag to false, await response
  (OPC-10000-4 §5.6.4)
- [ ] T020 [US1] Implement `mu_client_disconnect` in `src/client/client.c`
  — close SecureChannel via T016, close TCP via transport, reset state
  to DISCONNECTED (separate API from close_session per MEDIUM-006)
- [ ] T021 [US1] Implement connection loss detection in `src/client/client.c`
...
- [ ] T022 [P] [US2] Add read tests in `tests/unit/test_nano_client.c` — read
  known variable and verify value; read non-existent node and verify error
  (OPC-10000-4 §5.10.2)
- [ ] T023 [US2] Implement `mu_client_read` in `src/client/client_read.c`
  — construct ReadRequest with node IDs, timestampsToReturn, maxAge;
  send; parse ReadResponse; return values with StatusCodes
  (OPC-10000-4 §5.10.2)
- [ ] T024 [US2] Handle error responses in client_read — Bad_NodeIdUnknown,
...
- [ ] T025 [P] [US3] Add browse tests in `tests/unit/test_nano_client.c` — browse
  Objects folder, verify child nodes; test BrowseNext with continuation;
  test TranslateBrowsePaths with known path (OPC-10000-4 §5.8.2-4)
- [ ] T026 [US3] Implement `mu_client_browse` in `src/client/client_browse.c`
...
- [ ] T027 [US3] Implement `mu_client_browse_next` in `src/client/client_browse.c`
  — if continuation point in response, send BrowseNextRequest to get
  next batch (OPC-10000-4 §5.8.3)
- [ ] T028 [US3] Implement `mu_client_translate_browse_paths_to_node_ids` in
...
- [X] T030 Run full test suite to confirm no server regression
- [X] T031 Run formatting and static analysis (clang-format, cppcheck)
...
- [X] T035 Verify zero-heap — compile client with `MUC_OPCUA_ALLOW_HEAP=OFF`,
...
- [X] T037 Update docs/traceability/ with client service-to-section mappings
...
- T009, T010, T011, T012 can run alongside T013-T021 after T006+T007
- T022 can run before T023-T024
- T025 can run before T026-T028
