...
static opcua_statuscode_t handle_update_certificate(mu_server_t *server, void *context,
    ...
    if (output_args_count != NULL) *output_args_count = 0u;

    const mu_certificate_manager_adapter_t *adapter = adapter_of(server);
    if (adapter == NULL) return MU_STATUS_BAD_NOTSUPPORTED;
...
static opcua_statuscode_t handle_apply_changes(mu_server_t *server, void *context,
                                               const mu_nodeid_t *object_id,
                                               const mu_nodeid_t *method_id,
                                               const mu_variant_t *input_args,
                                               size_t input_args_count,
...
opcua_statuscode_t mu_certificate_push_register(mu_server_t *server) {
    if (server == NULL) return MU_STATUS_BAD_INTERNALERROR;
    ...
    s = mu_server_register_method_callback(server, &update_cert_id, handle_update_certificate,
                                           NULL, NULL, 0, NULL, 0, true);
    if (s != MU_STATUS_GOOD) return s;
    s = mu_server_register_method_callback(server, &apply_changes_id, handle_apply_changes,
                                           NULL, NULL, 0, NULL, 0, true);
    return s;
}
