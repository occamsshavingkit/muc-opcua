...
static opcua_statuscode_t write_header(mu_binary_writer_t *w, mu_security_policy_id_t policy, opcua_uint32_t scid,
    ...
    mu_bytestring_t cert = {sender_cert ? (opcua_int32_t)(sender_cert_len > (size_t)INT32_MAX ? INT32_MAX : sender_cert_len) : -1, sender_cert};
    s = mu_binary_write_bytestring(w, &cert);
    if (s != MU_STATUS_GOOD) {
        return s;
    }

    mu_bytestring_t thumb = {receiver_thumbprint ? (opcua_int32_t)(thumb_len > (size_t)INT32_MAX ? INT32_MAX : thumb_len) : -1, receiver_thumbprint};
