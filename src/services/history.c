...
opcua_statuscode_t mu_history_read_request_decode(mu_binary_reader_t *reader, mu_history_read_request_t *req,
    ...
    if ((encoding_mask & 0x01) == 0) return MU_STATUS_BAD_NOTSUPPORTED;
    ...
    s = mu_binary_read_boolean(reader, &req->details.is_read_modified);
    if (s != MU_STATUS_GOOD) return s;

    s = mu_binary_read_int64(reader, &req->details.start_time);
    if (s != MU_STATUS_GOOD) return s;

    s = mu_binary_read_int64(reader, &req->details.end_time);
    if (s != MU_STATUS_GOOD) return s;

    s = mu_binary_read_uint32(reader, &req->details.num_values_per_node);
    if (s != MU_STATUS_GOOD) return s;

    s = mu_binary_read_boolean(reader, &req->details.return_bounds);
    if (s != MU_STATUS_GOOD) return s;
    ...
    for (size_t i = 0; i < req->num_nodes_to_read; ++i) {
        ...
        s = mu_binary_read_bytestring(reader, &node->continuation_point);
        if (s != MU_STATUS_GOOD) return s;
    }
...
opcua_statuscode_t mu_history_update_request_decode(mu_binary_reader_t *reader, mu_history_update_request_t *req,
    ...
    if (!reader || !req || !items_array) return MU_STATUS_BAD_INTERNALERROR;
