...
opcua_statuscode_t mu_binary_write_datavalue(mu_binary_writer_t *writer, const mu_datavalue_t *value) {
    ...
    if (value->has_value) encoding_mask |= 0x01;
    if (value->has_status) encoding_mask |= 0x02;
    if (value->has_source_timestamp) encoding_mask |= 0x04;
    if (value->has_server_timestamp) encoding_mask |= 0x08;
