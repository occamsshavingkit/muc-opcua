...
static opcua_statuscode_t arduino_tcp_read(void *context, void *connection_handle, opcua_byte_t *buffer,
...
static opcua_statuscode_t arduino_tcp_write(void *context, void *connection_handle, const opcua_byte_t *buffer,
...
static opcua_statuscode_t arduino_generate_random(void *context, opcua_byte_t *buffer, size_t length) {
    (void)context;
    if (buffer) {
        (void)memset(buffer, 0, length); /* Stub */
    }
    return MU_STATUS_GOOD;
}

void mu_arduino_adapter_init(mu_tcp_adapter_t *tcp_adapter, mu_time_adapter_t *time_adapter,
