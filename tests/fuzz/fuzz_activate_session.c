...
static bool prepare_activate_session_server(mu_server_t *server)
{
    ...
    return mu_session_create(&server->sessions[0], 0u, &revised_timeout,
                             &session_id, &auth_token) == MU_STATUS_GOOD;
}

static opcua_statuscode_t write_activate_prefix(opcua_byte_t *buffer,
                                                size_t capacity,
...
static size_t build_structured_request(const uint8_t *data,
                                       size_t size,
                                       opcua_byte_t *request,
                                       size_t capacity)
{
    ...
    static const opcua_byte_t seed_tail[] = {
        ...
        0x02, 0x00, 0x00, 0x00, 0x58, 0x59,
        0x01, 0x00, 0x00, 0x00,
        0x05, 0x00, 0x00, 0x00, 0x65, 0x6e, 0x2d, 0x55, 0x53,
        0x01, 0x00, 0x41, 0x01,
        0x01, 0x0d, 0x00, 0x00, 0x00,
        0x09, 0x00, 0x00, 0x00, 0x61, 0x6e, 0x6f, 0x6e, 0x79,
        0x6d, 0x6f, 0x75, 0x73,
        0x06, 0x00, 0x00, 0x00, 0x73, 0x69, 0x67, 0x61, 0x6c, 0x67,
...
static void dispatch_activate_session_body(const opcua_byte_t *request_body,
                                           size_t request_length)
{
    ...
    (void)mu_service_dispatch(&server, MU_ID_ACTIVATESESSIONREQUEST,
...
int LLVMFuzzerTestOneInput(const uint8_t *data, size_t size)
{
    ...
    structured_length = build_structured_request(data,
                                                 size,
                                                 structured_request,
                                                 sizeof(structured_request));
