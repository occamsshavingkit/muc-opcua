...
void test_mu_alarms_acknowledge_method_call(void) {
    ...
    opcua_statuscode_t status = mu_alarms_conditions_method_dispatch(&server, &method_id, &alarm_id.node_id, 0, NULL, &output_args_count, output_args);
...
void test_mu_alarms_dialog_respond_method(void) {
    ...
    mu_variant_t output_args[2];
    size_t output_args_count = 0;
    opcua_statuscode_t status = mu_alarms_conditions_method_dispatch(&server, &method_id, &dialog_id.node_id, 1, input_args, &output_args_count, output_args);
