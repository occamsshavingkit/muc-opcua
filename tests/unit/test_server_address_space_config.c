 ...
 * order[MU_INTERN_MAX_ADDRESS_SPACE_NODES] array. A space larger than the cap must be
...
static mu_node_t s_nodes[MU_INTERN_MAX_ADDRESS_SPACE_NODES + 1];
...
void setUp(void) {
    ...
    for (size_t i = 0; i < (MU_INTERN_MAX_ADDRESS_SPACE_NODES + 1); ++i) {
...
void test_server_init_accepts_max_nodes(void) {
    s_space.node_count = MU_INTERN_MAX_ADDRESS_SPACE_NODES;
...
void test_server_init_rejects_too_many_nodes(void) {
    s_space.node_count = MU_INTERN_MAX_ADDRESS_SPACE_NODES + 1;
    mu_server_t *server = NULL;
