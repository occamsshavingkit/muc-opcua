...
void test_mbedtls_ecc_all(void) {
    ...
    TEST_ASSERT_EQUAL(MU_STATUS_GOOD,
                      mu_mbedtls_crypto_adapter_set_ecc_identity(&mbed_crypto, MU_ECC_CURVE_NISTP256, ecc_cert_der,
                                                                 (size_t)ecc_cert_len, ecc_key_der,
                                                                 (size_t)ecc_key_len));
    ...
    TEST_ASSERT_EQUAL(MU_STATUS_GOOD,
                      host_crypto.get_own_ecc_certificate(host_crypto.context, MU_ECC_CURVE_NISTP256, &host_ecc_cert,
                                                          &host_ecc_cert_len));
    sig_len = sizeof(sig);
    TEST_ASSERT_EQUAL(MU_STATUS_GOOD,
                      host_crypto.ecc_sign(host_crypto.context, MU_ECC_CURVE_NISTP256, msg, sizeof(msg), sig, &sig_len));
    TEST_ASSERT_EQUAL(MU_ECC_SIGNATURE_LENGTH, sig_len);
    TEST_ASSERT_EQUAL(MU_STATUS_GOOD,
                      mbed_crypto.ecc_verify(mbed_crypto.context, MU_ECC_CURVE_NISTP256, host_ecc_cert,
                                             host_ecc_cert_len, msg, sizeof(msg), sig, sig_len));

    /* 2. mbedTLS signs (its independent cert), host verifies. */
    sig_len = sizeof(sig);
    TEST_ASSERT_EQUAL(MU_STATUS_GOOD,
                      mbed_crypto.ecc_sign(mbed_crypto.context, MU_ECC_CURVE_NISTP256, msg, sizeof(msg), sig, &sig_len));
    TEST_ASSERT_EQUAL(MU_ECC_SIGNATURE_LENGTH, sig_len);
    TEST_ASSERT_EQUAL(MU_STATUS_GOOD,
                      host_crypto.ecc_verify(host_crypto.context, MU_ECC_CURVE_NISTP256, (const opcua_byte_t *)ecc_cert_der,
                                             (size_t)ecc_cert_len, msg, sizeof(msg), sig, sig_len));
    ...
    TEST_ASSERT_EQUAL(MU_STATUS_GOOD, mbed_crypto.ecc_ecdh_derive(mbed_crypto.context, MU_ECC_CURVE_NISTP256, kp_m, pub_h,
                                                                  pub_h_len, shared_m, &shared_m_len));
    TEST_ASSERT_EQUAL(MU_STATUS_GOOD, host_crypto.ecc_ecdh_derive(host_crypto.context, MU_ECC_CURVE_NISTP256, kp_h, pub_m,
                                                                  pub_m_len, shared_h, &shared_h_len));
    ...
    TEST_ASSERT_EQUAL(MU_STATUS_GOOD,
                      mbed_crypto.chacha20_poly1305_encrypt(mbed_crypto.context, cc_key, cc_nonce, cc_aad, sizeof(cc_aad),
                                                            cc_pt, sizeof(cc_pt), cc_ct, cc_tag));
    TEST_ASSERT_EQUAL(MU_STATUS_GOOD,
    ...
    sig_len = sizeof(sig);
    TEST_ASSERT_NOT_EQUAL(MU_STATUS_GOOD, mbed_crypto.ecc_sign(mbed_crypto.context, MU_ECC_CURVE_25519, msg, sizeof(msg),
                                                              sig, &sig_len));
