...
opcua_statuscode_t handle_history_read(mu_server_t *server, mu_binary_reader_t *r, mu_binary_writer_t *w, size_t *response_length);
opcua_statuscode_t handle_history_update(mu_server_t *server, mu_binary_reader_t *r, mu_binary_writer_t *w, size_t *response_length);
...
static opcua_statuscode_t mock_read_raw_modified(
    void *context,
    ...
    (void)context; (void)is_read_modified; (void)end_time; (void)num_values_per_node;
    (void)return_bounds; (void)cp_in; (void)cp_in_length; (void)cp_out;
...
void test_history_read_dispatch(void) {
    ...
    TEST_ASSERT_EQUAL(MU_STATUS_GOOD, mu_binary_write_nodeid(&req_w, &(mu_nodeid_t){ .namespace_index = 0, .identifier_type = MU_NODEID_NUMERIC, .identifier.numeric = 0 })); // auth token
    ...
    TEST_ASSERT_EQUAL(MU_STATUS_GOOD, mu_binary_write_nodeid(&req_w, &(mu_nodeid_t){ .namespace_index = 0, .identifier_type = MU_NODEID_NUMERIC, .identifier.numeric = 0 })); // additionalHeader
    TEST_ASSERT_EQUAL(MU_STATUS_GOOD, mu_binary_write_byte(&req_w, 0)); // encoding mask
 
    // HistoryReadRequest
    // historyReadDetails (ExtensionObject)
    TEST_ASSERT_EQUAL(MU_STATUS_GOOD, mu_binary_write_nodeid(&req_w, &(mu_nodeid_t){ .namespace_index = 0, .identifier_type = MU_NODEID_NUMERIC, .identifier.numeric = MU_ID_READRAWMODIFIEDDETAILS_ENCODING_DEFAULTBINARY }));
    ...
    TEST_ASSERT_EQUAL(MU_STATUS_GOOD, mu_binary_write_nodeid(&req_w, &(mu_nodeid_t){ .namespace_index = 2, .identifier_type = MU_NODEID_NUMERIC, .identifier.numeric = 1001 }));
...
void test_history_read_pagination(void) {
    ...
    TEST_ASSERT_EQUAL(MU_STATUS_GOOD, mu_binary_write_int32(&req_w, 1));
    TEST_ASSERT_EQUAL(MU_STATUS_GOOD, mu_binary_write_nodeid(&req_w, &(mu_nodeid_t){ .namespace_index = 2, .identifier_type = MU_NODEID_NUMERIC, .identifier.numeric = 1002 }));
    TEST_ASSERT_EQUAL(MU_STATUS_GOOD, mu_binary_write_string(&req_w, &(mu_string_t){-1, NULL})); // indexRange
    TEST_ASSERT_EQUAL(MU_STATUS_GOOD, mu_binary_write_uint16(&req_w, 0)); // namespaceIndex
    TEST_ASSERT_EQUAL(MU_STATUS_GOOD, mu_binary_write_string(&req_w, &(mu_string_t){-1, NULL})); // name
...
static opcua_statuscode_t mock_update_data(
    void *context,
    const mu_nodeid_t *node_id,
...
static opcua_statuscode_t mock_delete_raw_modified(
    void *context,
    const mu_nodeid_t *node_id,
...
void test_history_update_decode(void) {
    ...
    TEST_ASSERT_EQUAL(MU_STATUS_GOOD, mu_binary_write_nodeid(&w, &(mu_nodeid_t){ .namespace_index = 0, .identifier_type = MU_NODEID_NUMERIC, .identifier.numeric = MU_ID_UPDATEDATADETAILS_ENCODING_DEFAULTBINARY }));
    ...
    TEST_ASSERT_EQUAL(MU_STATUS_GOOD, mu_binary_write_nodeid(&w, &(mu_nodeid_t){ .namespace_index = 2, .identifier_type = MU_NODEID_NUMERIC, .identifier.numeric = 1001 }));
    ...
    TEST_ASSERT_EQUAL(MU_STATUS_GOOD, mu_binary_write_nodeid(&w, &(mu_nodeid_t){ .namespace_index = 0, .identifier_type = MU_NODEID_NUMERIC, .identifier.numeric = MU_ID_DELETERAWMODIFIEDDETAILS_ENCODING_DEFAULTBINARY }));
    TEST_ASSERT_EQUAL(MU_STATUS_GOOD, mu_binary_write_byte(&w, 0x01)); // encoding mask = ByteString
...
void test_history_update_dispatch(void) {
    ...
    // 1. UpdateDataDetails
    TEST_ASSERT_EQUAL(MU_STATUS_GOOD, mu_binary_write_nodeid(&req_w, &(mu_nodeid_t){ .namespace_index = 0, .identifier_type = MU_NODEID_NUMERIC, .identifier.numeric = MU_ID_UPDATEDATADETAILS_ENCODING_DEFAULTBINARY }));
    ...
    TEST_ASSERT_EQUAL(MU_STATUS_GOOD, mu_binary_write_nodeid(&req_w, &(mu_nodeid_t){ .namespace_index = 0, .identifier_type = MU_NODEID_NUMERIC, .identifier.numeric = MU_ID_DELETERAWMODIFIEDDETAILS_ENCODING_DEFAULTBINARY }));
    TEST_ASSERT_EQUAL(MU_STATUS_GOOD, mu_binary_write_byte(&req_w, 0x01)); // encoding mask = ByteString
