...
/* SecureChannelId assigned by the server in the OPN response; MSG chunks must
   carry this id (OPC-10000-6 section 6.7.2.2). */
...
/* Patch the SecureChannelId field (offset 8, LE) in every enqueued MSG/CLO
   chunk so they carry the server-assigned id rather than the placeholder. */
...
static size_t build_msg(opcua_byte_t *out, size_t cap, opcua_uint32_t seq, opcua_uint32_t reqid,
    ...
    mu_binary_write_uint32(&w, s_channel_id);                    /* SecureChannelId */
