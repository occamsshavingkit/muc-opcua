...
void test_server_handshake_connect_browse_read(void) {
    ...
    static const mu_node_t nodes[] = {
        { { 0, MU_NODEID_NUMERIC, { 85 } }, MU_NODECLASS_OBJECT,
          { 7, (const opcua_byte_t *)"Objects" }, { 7, (const opcua_byte_t *)"Objects" }, obj_refs, 1, NULL },
    ...
    { opcua_int32_t nres; mu_binary_read_int32(&body, &nres); TEST_ASSERT_EQUAL(1, nres);
      ...
      opcua_int32_t cp; mu_binary_read_int32(&body, &cp);
      opcua_int32_t nrefs; mu_binary_read_int32(&body, &nrefs); TEST_ASSERT_EQUAL(1, nrefs); }
