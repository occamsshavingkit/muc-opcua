...
| DataChangeFilter percent deadband | §5.13.2, §5.13.3, §7.22.2, §7.38.2 | **With the Data Access facet (`MUC_OPCUA_DATA_ACCESS`, spec 060):** supported — accepted on an AnalogItem with EURange, else `Bad_DeadbandFilterInvalid`. **Without it:** per-item `Bad_MonitoredItemFilterUnsupported` (outside the base data-change subset). See [data-access.md](data-access.md). | `tests/integration/test_subscriptions.c`, `tests/unit/test_subscriptions_errors.c` |
