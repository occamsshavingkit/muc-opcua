...
def test_resolve_into_maps_graph_cu_and_leaves_absent():
    ...
    assert st["depends_on"] == ["MUC_OPCUA_FACET_EXPOSES_TYPE_SYSTEM_SERVER"]  # BASE_TYPES dropped
    assert "depends_on_op" not in st or st["depends_on_op"] is None
    assert st["profile_defaults"]["embedded"] is True and st["profile_defaults"]["nano"] is False
    assert st["profile_defaults"]["full"] is True  # derived from implementation_state=claimed

    sx = next(i for i in manifest["items"] if i["id"] == "service_x")
    assert sx["depends_on"] == ["HAND_AUTHORED"]
    assert sx["profile_defaults"] == {
        "nano": True, "micro": True, "embedded": True, "standard": True,
        "full": True, "custom": True,
    }


def test_resolve_into_skips_non_conformance_unit_items():
    graph = _full_graph()
...
def test_resolve_into_full_false_when_not_implemented():
    graph = _full_graph()
    manifest = {"items": [
        {"id": "opc_facet_1219", "kind": "facet",
         "kconfig_symbol": "MUC_OPCUA_FACET_EXPOSES_TYPE_SYSTEM_SERVER",
    ...
    d.resolve_into(manifest, graph)
    it = manifest["items"][1]
    assert it["profile_defaults"]["full"] is False
    assert it["profile_defaults"]["custom"] is False  # defaulted, not present before
